...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!email || !password) {
      return toast.error('Please fill in all requested validation fields');
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Authentication clearance failed');
      }

      login(data.token);
      toast.success('Identity validation approved!');
      navigate(from, { replace: true });
    } catch (err) {
      toast.error(err.message || 'Network authentication mismatch occurred');
    } finally {
        ...
        <div className="p-8 md:p-10">
          ...
          <div className="mb-8 text-center">
            ...
            <p className="mt-3 text-sm leading-relaxed text-charcoal/60">
              Authenticate your credentials to access encrypted uploads and secure transfer operations.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              ...
              className="group mt-2 flex w-full items-center justify-center gap-2 rounded-2xl bg-sage py-4 font-semibold text-white shadow-xl shadow-sage/15 transition-all duration-300 hover:-translate-y-0.5 hover:bg-[#243c33] disabled:cursor-not-allowed disabled:opacity-60"
            >
              {formLoading ? (
